...
class PublishConfig:
    def __init__(self):
        ...
        self.table_name = "publish_config"

    def get_config_using_query_params(self, query_parameters):
        connection_obj = get_connection(self.database_config)
        try:
            sql = f"SELECT * FROM {self.table_name} a WHERE a.bank = %(bank)s"
            if "merge_config_name" in query_parameters:
                sql += " AND a.merge_config_name = %(merge_config_name)s"
            if "environment" in query_parameters:
                sql += " AND a.environment = %(environment)s"
            if "project" in query_parameters:
                sql += " AND a.project = %(project)s"
            if "merge_config_version" in query_parameters:
                sql += " AND a.merge_config_version = %(merge_config_version)s"
            if "publish_config_version" in query_parameters:
                sql += " AND a.publish_config_version = %(publish_config_version)s"
    ...
    def latest_config_version(self, query_parameters):
        connection_obj = get_connection(self.database_config)
        try:
            sql = f"SELECT publish_config_version FROM {self.table_name} a WHERE a.bank = %(bank)s"
            if "merge_config_name" in query_parameters:
                sql += " AND a.merge_config_name = %(merge_config_name)s"
            if "project" in query_parameters:
                sql += " AND a.project = %(project)s"
            ...
            if "publish_config_version" in query_parameters:
                sql += " AND a.publish_config_version = %(publish_config_version)s"
            sql += " ORDER BY publish_config_version DESC LIMIT 1"
            result = execute_select(sql, query_parameters, connection_obj)
            if result is None or len(result) == 0:
                return 0
            return result[0].get("publish_config_version")
        except Exception as e:
            logger.error(
                    f'Error thrown while selecting query : {sql}, message: {str(e)}, log_key: get_attempt_by_rid')
            return None
    ...
    def mark_all_version_inactive(self, bank, environment, project, merge_config_name, merge_config_version):
        connection_obj = get_connection(self.database_config)
        try:
            where_dict = {
                "bank": bank,
    ...
    def mark_publish_version_active(self, bank, environment, project, merge_config_name, merge_config_version, publish_config_version):
        connection_obj = get_connection(self.database_config)
        try:
            where_dict = {
                "bank": bank,
                ...
                "project": project,
                "merge_config_name": merge_config_name,
                "merge_config_version": merge_config_version,
                "publish_config_version": publish_config_version
            }
            upd_dict = {
                "active": 1
            }
    ...
    def mark_publish_version_tested(self, bank, merge_config_name, publish_config_version):
        connection_obj = get_connection(self.database_config)
        try:
            where_dict = {
                "bank": bank,
            ...
            logger.error(
                    f'Error thrown while updating query : {self.table_name}, message: {str(e)}, log_key: update_published_version_via_id')
            return None
